      ...
      <div className="container">
        <div data-aos="zoom-in-up" id="notes" className="w-full px-5 pb-14 pt-3 md:px-10 md:pb-12 mt-7 md:mt-16 md:w-3/4 md:mx-auto text-slate-600 border border-slate-300 border-dashed rounded-lg shadow-lg bg-white dark:border-slate-500 dark:bg-slate-700">
          <div className="flex justify-end">
            <Link to={`/notes/edit/${id}`} className="right-0 rounded-lg py-2 outline outline-none outline-1 hover:outline-blue-300"><FiEdit className="text-2xl md:text-3xl text-blue-500 mx-3 hover:opacity-70" /></Link>
          </div>
          ...
          <p className="text-lg dark:text-slate-300">{parser(body)}</p>
        </div>
      </div>
    </section>
  )
